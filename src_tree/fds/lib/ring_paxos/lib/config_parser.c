...
acceptor_info * lpconfig_get_acceptor_info(config_mngr * cfg, acceptor_id_t acceptor) {
    assert(acceptor <= MAX_ACCEPTORS);    
	assert(cfg->initialized);

    acceptor_info * ai = &cfg->acc_infos[acceptor];
    assert(ai->id <= MAX_ACCEPTORS);
    return ai;
}

char * lpconfig_get_self_ip_addr(config_mngr * cfg) {
    return (lpconfig_get_acceptor_info(cfg, lpconfig_get_self_acceptor_id(cfg)))->ip_addr;
}

char * lpconfig_get_ring_inbound_addr(config_mngr * cfg) {
    return lpconfig_get_self_ip_addr(cfg);
}

int lpconfig_get_ring_inbound_port(config_mngr * cfg) {
    acceptor_info * my_info = &cfg->acc_infos[lpconfig_get_self_acceptor_id(cfg)];
...
char * lpconfig_get_ip_addr_of(config_mngr * cfg, acceptor_id_t acceptor) {
    return (lpconfig_get_acceptor_info(cfg, acceptor))->ip_addr;
}

int lpconfig_get_learners_port_of(config_mngr * cfg, acceptor_id_t acceptor) {
    return (lpconfig_get_acceptor_info(cfg, acceptor))->learners_port;
}

int lpconfig_get_ring_port_of(config_mngr * cfg, acceptor_id_t acceptor) {
    return (lpconfig_get_acceptor_info(cfg, acceptor))->inbound_port;
}
...
static int validate_socket_buffer_size(config_mngr * cfg) {
	...
	if(lpconfig_get_socket_buffers_size(cfg) > value) {
...
static int validate_config(config_mngr * cfg) {

	//Multicaster address/port
	VALIDATE_ADDRESS(lpconfig_get_mcast_addr(cfg));
	VALIDATE_PORTNUMBER(lpconfig_get_mcast_port(cfg));

	// Multicaster time variables
	VALIDATE_TIMEVAL_NONZERO(lpconfig_get_p1_interval(cfg));
	VALIDATE_TIMEVAL_NONZERO(lpconfig_get_p2_interval(cfg));
	VALIDATE_TIMEVAL_NONZERO_OR_DEFAULT(lpconfig_get_mcaster_clock_interval(cfg), 0, 100000);
	VALIDATE_TIMEVAL_GREATER_EQUAL_THAN(lpconfig_get_p2_interval(cfg), lpconfig_get_mcaster_clock_interval(cfg));
	VALIDATE_TIMEVAL_GREATER_EQUAL_THAN(lpconfig_get_p1_interval(cfg), lpconfig_get_mcaster_clock_interval(cfg));
	
	// Learners intervals
	VALIDATE_TIMEVAL_NONZERO_OR_DEFAULT(lpconfig_get_delivery_check_interval(cfg), 1, 0);
	VALIDATE_TIMEVAL_NONZERO_OR_DEFAULT(lpconfig_get_retransmit_request_interval(cfg), 1, 0);
	VALIDATE_TIMEVAL_GREATER_EQUAL_THAN(lpconfig_get_retransmit_request_interval(cfg), lpconfig_get_delivery_check_interval(cfg));
		
	// Acceptors number and quorum size
	VALIDATE_INT_NONZERO(lpconfig_get_quorum_size(cfg));	
	VALIDATE_INT_COMPARISON(cfg->acceptors_count, (int)lpconfig_get_quorum_size(cfg), >=);

	// Network flush
	VALIDATE_INT_NONZERO_OR_DEFAULT(cfg->default_autoflush_interval, 50);

    // Paxos-related parameters
	VALIDATE_INT_NONZERO_OR_DEFAULT(cfg->working_set_size, 100);
	VALIDATE_INT_NONZERO_OR_DEFAULT(cfg->max_active_instances, 25);
	VALIDATE_INT_NONZERO_OR_DEFAULT(cfg->preexecution_window_size, 50);
	VALIDATE_INT_NONZERO_OR_DEFAULT(cfg->max_p2_open_per_iteration, 10);
	VALIDATE_INT_NONZERO_OR_DEFAULT(cfg->max_client_values_queue_size, 100);
	
	
	// Validate other params
	
	//Check default size of OS socket buffers
    if(lpconfig_get_socket_buffers_size(cfg) <= 0) {
		LOG_MSG(WARNING, ("WARNING: socket_buffers_size is not set in configuration!\n"));
		LOG_MSG(WARNING, ("Default value will be used\n"));
	} else if (validate_socket_buffer_size(cfg) == -1) {
        goto VALIDATE_ERROR_LABEL;		
	}
	
	//Make sure Working set size is big enough
	VALIDATE_INT_COMPARISON(lpconfig_get_working_set_size(cfg), lpconfig_get_max_active_instances(cfg), >);
	if (lpconfig_get_max_active_instances(cfg)*2 >= lpconfig_get_working_set_size(cfg)) {
		LOG_MSG(WARNING, ("WARNING: max_active_instances is too big w.r.t working_set_size\n"));
	}	
	if(lpconfig_get_working_set_size(cfg) <= 
		(lpconfig_get_preexecution_window_size(cfg) + lpconfig_get_max_active_instances(cfg))) {
	        ...
	        printf(" (You should increase the former (%d) or increase one of the latter (%d+%d))\n", 
				lpconfig_get_working_set_size(cfg), lpconfig_get_preexecution_window_size(cfg), 
				lpconfig_get_max_active_instances(cfg));
			goto VALIDATE_ERROR_LABEL;		
	}
    ...
    for(i = 1; i < MAX_ACCEPTORS; i++) {
        ai = lpconfig_get_acceptor_info(cfg, i);
		...
		VALIDATE_ADDRESS(lpconfig_get_ip_addr_of(cfg, i));
        
        //Port numbers below 1024 require superuser privileges
		VALIDATE_PORTNUMBER(lpconfig_get_learners_port_of(cfg, i));
		VALIDATE_PORTNUMBER(lpconfig_get_ring_port_of(cfg, i));
		LOG_MSG(DEBUG, ("Acceptor info slot %d is valid\n", i));
	}

    //Make sure there are no acceptors with the same ID
	VALIDATE_INT_COMPARISON(lpconfig_get_acceptors_count(cfg), count, ==);
