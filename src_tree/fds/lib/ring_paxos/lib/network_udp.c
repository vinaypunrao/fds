...
udp_read_callback_wrapper(int fd, short event, void *arg)
{
    ...
    while(current_offset < udatasize) {
		...
		if((random() % 100) < MESSAGE_LOSS_PROBABILITY) {
	        ...
	        ur->cb(&next_msg->data, next_msg->size, next_msg->type, ur->cb_arg);
		}
#else
	...
	)
{
	...
	udp_receiver * ur = udp_receiver_init(addr_str, port, cb, cb_arg, cfg);
	if(ur == NULL) {
		return NULL;
	}
	...
	)
{
	...
	LOG_MSG(DEBUG, ("Creating sender for multicast address %s:%d\n", addr_str, port));
			...
			//Create a normal udp receiver
	udp_sender * us = udp_sender_init(addr_str, port, cfg);
	if(us == NULL) {
		return NULL;
	}
...
void udp_autoflush_cb(void * us) {
	assert(((udp_sender*)us)->initialized);
...
void udp_sender_enable_default_autoflush(udp_sender * us) {
	...
	udp_sender_enable_autoflush(us, lpconfig_get_default_autoflush_interval(us->cfg));
}
