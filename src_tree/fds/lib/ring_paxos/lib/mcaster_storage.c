...
static void create_value_key(mcaster_storage_mngr * msm, command_id * cmd_key) {
    ...
    cmd_key->mcaster_id = lpconfig_get_self_acceptor_id(msm->cfg);
    cmd_key->mcaster_incarnation = lpconfig_get_incarnation_number(msm->cfg);
...
mcaster_storage_mngr * mcaster_storage_init(config_mngr * cfg, clival_mngr * cvm) {
	
	int number_of_entries = lpconfig_get_working_set_size(cfg)*2;
    ...
    size_t cmd_size) 
{
    ...
    bool success;
    success = cvm_push_back_value(msm->cvm, mir->assigned_cmd_value, mir->assigned_cmd_size);
    assert(success == true);
