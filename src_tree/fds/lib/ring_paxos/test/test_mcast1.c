...
int main (int argc, char const *argv[]) {
    ...
    int port = lpconfig_get_mcast_port(cfg);
    char * mcast_addr = lpconfig_get_mcast_addr(cfg);

    event_init();
    
    udp_receiver * ur = mcast_receiver_init(mcast_addr, port, handle_msg, NULL, cfg);
    assert(ur != NULL);

    udp_sender * us = mcast_sender_init(mcast_addr, port, cfg);
    assert(us != NULL);
