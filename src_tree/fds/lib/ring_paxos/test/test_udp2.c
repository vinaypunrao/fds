...
void pre_receive(void * arg) {
	UNUSED_ARG(arg);
	pre_recv_count += 1;
}
void post_receive(void * arg) {
	UNUSED_ARG(arg);
	post_recv_count += 1;
}
void post_deliver(void * arg) {
	UNUSED_ARG(arg);
...
int main (int argc, char const *argv[]) {
    ...
    udp_receiver * ur = udp_receiver_init(NULL, port, handle_msg, NULL, cfg);
    assert(ur != NULL);
    ...
    udp_sender * us = udp_sender_init("127.0.0.1", port, cfg);
    assert(us != NULL);
